	...
	{														
		...
		if (GetCommState(hCOM, &dcb))
		{
			if (BuildCommDCB("baud=115200 parity=N data=8 stop=1 xon=off", &dcb))
			{
                ...
                {
                   SetupComm(hCOM, 16384, 16384);									// Применяем новые настройки инициализируем COM и устанавливаем размеры буферов.
                }
...
int WriteBreak()
{
	int Address = 0x90000050;												// Запись аппаратных точек.
	for ( int i = 0; i < 2; i++)
	{	
...
int Run()
{
	...
	for (int i = 0; i < 4; i++)
		if ((*UIntData == ArrayWatch[0][i]) || ((*UIntData == (ArrayBreak[0][i] & ( ~ 0x1)))))
			printf("Программа достигла точки остановки: 0x%08X\n", *UIntData );
	return 0;
}
...
int Step()
{
	...
	{
		ReadSwap(0x90400010, UIntAddress );	
		D=0x00010001;
		swp(D);
		Wmem(0x90000020, 1, (unsigned char *)&D);
		D=0x00010000;
		swp(D);
		Wmem(0x90000020, 1, (unsigned char *)&D);
		printf("0x%08X ", UIntAddress );									// Вывод дзассемблируемых данных.
		ReadSwap(UIntAddress, UIntData );	
		Disassemble(UIntData, UIntAddress, (char *)ReadBuffer, false);					
		cout << " " << ReadBuffer;
	}
	return 0;
}
