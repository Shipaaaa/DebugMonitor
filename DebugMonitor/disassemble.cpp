...
{			//fmt2
	...
	int cond=(int)(code>>25)&15;	//0x1E000000
	int a=(int)(code>>29)&1;		//0x20000000
	int op2=(int)(code>>22)&7;		//0x1C00000
		if (a!=0)
			sprintf(text,
					"%-9s,a 0x%X\r\n",
					(op2==2?
...
{	//fmt3 op=2
	...
	switch (op3) {
		case 48:
			if (rd==0) {
				if (i!=0)
					sprintf(text,"wr       %s, %d, %y\r\n",regs[rs1],simm13);
				else
					sprintf(text,"wr       %s, %s, %y\r\n",regs[rs1],regs[rs2]);
			}
			else {
				if (i!=0)
					sprintf(text,"wr       %s, %d, %%asr%d\r\n",regs[rs1],simm13,rd);
				else
					sprintf(text,"wr       %s, %s, %%asr%d\r\n",regs[rs1],regs[rs2],rd);
			}			
			/*if (rs1!=0)
				if (i==0)
					sprintf(text,"wr       %s, %s, %%y\r\n",regs[rs1],regs[rs2]);
		...
		case 49:
			if (i!=0)
				sprintf(text,"wr       %s, %d, %%psr\r\n",regs[rs1],simm13);
			else
				sprintf(text,"wr       %s, %s, %%psr\r\n",regs[rs1],regs[rs2]);
			break;
		case 50:
			if (i!=0)
				sprintf(text,"wr       %s, %d, %%wim\r\n",regs[rs1],simm13);
			else
				sprintf(text,"wr       %s, %s, %%wim\r\n",regs[rs1],regs[rs2]);
			break;
		case 51:
			if (i!=0)
				sprintf(text,"wr       %s, %d, %%tbr\r\n",regs[rs1],simm13);
			else
				sprintf(text,"wr       %s, %s, %%tbr\r\n",regs[rs1],regs[rs2]);
			break;
...
{	//fmt3 op=2
	...
	if ((op3>0&&op3<7&&op3!=4)||(op3>16&&op3<24&&op3!=20)) {
		...
		if (i!=0)
			sprintf(text,"%-9s%s, %d, %s\r\n",arlog[op3],regs[rs1],simm13,regs[rd]);
		else
			sprintf(text,"%-9s%s, %s, %s\r\n",arlog[op3],regs[rs1],regs[rs2],regs[rd]);
	}
}
...
{	//fmt3 op=3
	...
	if ((code&0x2000)!=0)
		...
		sprintf(text,"%-9s[%s + %s] %s\r\n",ldst[op3],regs[rs1],regs[rs2],regs[rd]);
	else
		sprintf(text,
			"%-9s[%s + %s]%d, %s\r\n",
			ldst[op3],
...
{	//fmt3 op=3
	...
	if ((code&0x2000)!=0)
		...
		sprintf(text,"%-9s%s, [%s + %s]\r\n",ldst[op3],regs[rd],regs[rs1],regs[rs2]);
	else
		sprintf(text,"%-9s%s, [%s + %s]%d\r\n",ldst[op3],regs[rd],regs[rs1],regs[rs2],asi);
}
	...
	switch (op3) {
		case 36:
			if (i!=0)
				...
				sprintf(text,"st       %%f%d, [%s + %s]\r\n",rd,regs[rs1],regs[rs2]);
			break;
		case 52:
			if (i!=0)
				sprintf(text,
	...
	if (idx!=-1) {
		if (op3==52) {
			if ((opf>=65&&opf<=79)||opf==105||opf==110)
				sprintf(text,"%s %%f%d, %%f%d, %%f%d\r\n",instr[idx],rs1,rs2,rd);
			else
				sprintf(text,"%s %%f%d, %%f%d\r\n",instr[idx],rs2,rd);
		}
		else if (op3==53) {
			sprintf(text,"%s %%f%d, %%f%d\r\n",instr[idx],rs2,rd);
		}
...
void cpop(UInt32 code,char* text) 
{
	...
	int op3=(int)(code>>19)&63;
	if (op3==54)
		sprintf(text,"cpop1    %d,%%c%d, %%c%d, %%c%d\r\n",opc,rs1,rs2,rd);
	else if (op3==55)
		sprintf(text,"cpop2    %d, %%c%d, %%c%d, %%c%d\r\n",opc,rs1,rs2,rd);
}
...
//{
//	char* text;
//	Disassemble(0x30BFFFFD,0x400068DC,text,false);
//	puts(text);
//	getchar();
//	return 0;
//}